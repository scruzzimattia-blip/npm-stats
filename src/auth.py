...
_login_attempts: Dict[str, Tuple[int, datetime, datetime]] = defaultdict(
    lambda: (0, datetime.now(timezone.utc), datetime.fromtimestamp(0, timezone.utc))
)
        ...
        with st.container():
            ...
            with col2:
                ...
                totp_code = st.text_input(
                    "MFA Code (falls aktiviert)",
                    key="auth_totp",
                    help="Lass dieses Feld leer, wenn du kein MFA aktiviert hast.",
                )
