...
def render_geo_summary(df: pd.DataFrame, geo_stats: Optional[Dict[str, pd.DataFrame]] = None) -> None:
    ...
    if geo_stats and not geo_stats.get("countries", pd.DataFrame()).empty:
        ...
        display_df["Eindeutige IPs"] = (
            "N/A"  # We skipped COUNT(DISTINCT) for performance, show N/A or calculate differently
        )
        ...
        if not city_df.empty:
            ...
            st.dataframe(
                city_display[["Stadt", "Land", "Requests"]].head(15), use_container_width=True, hide_index=True
            )
        ...
        geo_agg = (
            ...
            .agg({"remote_addr": ["count", "nunique"], "status": lambda x: (x >= 400).sum()})
            .reset_index()
        )
        ...
        st.dataframe(
            geo_agg[["Land", "Requests", "Eindeutige IPs", "Fehlerrate"]], use_container_width=True, hide_index=True
        )
...
def render_npm_hosts_status() -> None:
    ...
    from ..utils.npm_sync import check_all_hosts_health, fetch_npm_proxy_hosts
            ...
            display_data.append(
                {
                    ...
                    "SSL": ssl_info,
                    "Response": f"{health.get('response_time', 0) * 1000:.0f}ms"
