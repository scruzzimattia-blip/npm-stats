...
class AIAssistant:
    ...
    def ask(self, question: str, chat_history: List[Dict[str, str]] = None) -> Optional[str]:
        ...
        system_prompt = (
            "Du bist der NPM Monitor KI-Assistent, ein hochspezialisierter "
            "Senior Security Expert für Nginx Webserver-Sicherheit. "
            "Deine Aufgabe ist es, dem Administrator zu helfen, Bedrohungen zu "
            "identifizieren, Traffic-Muster zu verstehen und Sicherheitslücken "
            "zu schließen. "
            "\n\nRichtlinien für deine Antworten:"
            "\n- Sei präzise, technisch versiert und proaktiv."
            "\n- Wenn du nach Angriffen gefragt wirst, analysiere die Daten auf "
            "Muster wie SQLi, Path Traversal oder Botnet-Scanning."
            "\n- Gib konkrete Handlungsempfehlungen (z.B. 'IP permanent blocken', "
            "'Pfade in Honey-Paths aufnehmen')."
            "\n- Nutze Markdown für eine übersichtliche Struktur "
            "(Listen, Fettdruck, Tabellen)."
            "\n- Beziehe dich direkt auf die unten stehenden System-Daten."
            "\n- Falls du einen Abuse-Report Entwurf erstellst, nutze ein "
            "professionelles englisches Format für den Provider."
                    ...
                    with conn.cursor(row_factory=psycopg_rows.dict_row) as cur:
                        ...
                        if rows:
                            ...
                            for r in rows:
                                ...
                                parts.append(
                                    f"- IP {r['remote_addr']}: {r['count']} Anfragen, "
                                    f"{r['errors']} Fehler. Ziele: {targets}"
                                )
            except Exception:
                pass
    ...
    def _get_system_context(self) -> str:
        ...
        try:
            ...
            context_parts.append(
                f"System-Status: Gesamt-Requests: {db_info['total_rows']}, Aktive Sperren: {db_info['blocked_count']}"
            )
        except Exception:
            pass
        ...
        try:
            ...
            if blocks:
                ...
                for b in blocks:
                    ...
                    context_parts.append(
                        f"- IP: {b['ip_address']}, Grund: {b['reason']}, Bis: {b['block_until']}{threat}"
                    )
        except Exception:
            pass
